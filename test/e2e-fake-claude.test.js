...
test("fake Claude node runs in worktree, validates, commits, and merges", async () => {
  ...
  await fs.writeFile(fake, `#!/bin/sh\nif [ "$1" = "--version" ]; then echo "fake-claude 1.0"; exit 0; fi\nif [ "$1" = "auth" ]; then echo '{"loggedIn":true,"authMethod":"test"}'; exit 0; fi\nprintf 'generated by fake claude\\n' > src/generated.txt\nexit 0\n`);
  ...
  const orchestrator = new Orchestrator({ ...DEFAULT_CONFIG, cwd, claudeCommand: fake, claudeArgs: ["{prompt}"], worktreeRoot: ".cco/worktrees", stateFile: ".cco/state.json", validationCommands: ["git diff --check"], taskTimeoutMs: 10000 });
  ...
  assert.equal(await fs.readFile(path.join(cwd, "src/generated.txt"), "utf8"), "generated by fake claude\n");
